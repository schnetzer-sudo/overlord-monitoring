 ...
 *
 * **Die Ausnahmen sind gezählt, nicht gewachsen** — Stand 22.09.2026 sind es
 * **zweihundertvier in fünfundzwanzig Dateien** (bis zum 21.09.2026 zweihunderteins in
 * vierundzwanzig; einer mehr in `tests/suchfeld-auswahl.test.tsx` und zwei in der neuen
 * `tests/kopfzeile.test.tsx`, beide E‑232). Diese Zahl wird an genau dieser Stelle geführt;
 ...
 * | `tests/suche-marken.test.tsx` *(13.08.2026, erweitert 08.09.2026, ein Fall neu gefasst 22.09.2026)* | 6 | die Regression zum Schlüssel `(typ, wert)` an den Marken, der unbekannte Typ als Nummer, und zweimal eine Regel, die **selbst** eine Klasse plus ein `title` ist: die Längenregel der Trefferspalte und der Kettenhinweis. **Seit dem 22.09.2026 (E‑231) ist der Fall zum Kettenhinweis eine Aussage über Abwesenheit:** Die Spalte „Kette" ist entfallen — keine Kopfzelle, keine Zelle mit einer Kettenrolle, obwohl `rollen` auf der Zeile steht; die Zahl bleibt sechs. **Seit Teil 2 der Property-Suche zwei mehr:** dieselbe Parameterform als BAM- und als Feld-Marke **ohne doppelten React-Schlüssel** (der Schlüssel trägt die Art voran), und eine Aussage über **Abwesenheit** — ohne Belegnummer gibt es die Spalte „Treffer" nicht, weder Überschrift noch Zelle (E‑110) |
 ...
 * | `tests/suchfeld-auswahl.test.tsx` *(09.09.2026, erweitert 22.09.2026)* | 7 | **Seit dem 22.09.2026 einer mehr (E‑232, `docs/bam-suche.md` §11.1):** Der Schalter der Typwahl hat keine feste Breite mehr — bei gewähltem langem Feld trägt er den vollen Namen im `title` und im zugänglichen Namen, die Kürzungsklasse an der Spanne, den Höchstanteil `max-w-1/2` und das Nachgeben `shrink`; die Eingabe ihre Mindestbreite. Regeln, die selbst Klassen sind; die Breite des Suchbereichs rechnet `jsdom` nicht und bleibt Sichtprüfung. **Die Auswahl neben dem Suchfeld in zwei Untermenüs (E‑112).** Drei Aussagen über den Baum, die keine reine Funktion trägt: Die **oberste Ebene** trägt genau drei Einträge — den typlosen und je einen Untermenü-Auslöser — und keine Überschrift mehr; ein Untermenü zeigt beim Öffnen **seine** Einträge und die des anderen **nicht**; und für einen Mandanten ohne Belegart (`WOC`) **fehlt** der Auslöser der Belegarten vollständig — Abwesenheit, nicht Leere. Dazu die **Verdrahtung**: Eine Wahl im zweiten Untermenü hebt die im ersten auf, am Schalter, am Auslöser und am Häkchen; und der Auslöser der Gruppe mit der Auswahl trägt den gewählten Eintrag **im zugänglichen Namen, ohne `aria-checked`** — er ist `menuitem` mit `aria-haspopup`, kein Radioeintrag. Und die Regression zum Schlüssel: gleichlautende Einträge in beiden Gruppen **ohne `console.error`**. Geöffnet wird über `pointerdown` und `ArrowRight`, Radix' eigene Wege ohne Zeitgeber (T1); `next/navigation` ist ersetzt, weil `useRouter` außerhalb des App-Routers wirft |
 *
 * | `tests/kopfzeile.test.tsx` *(22.09.2026)* | 2 | **Wer in der Kopfzeile den freien Raum bekommt** (E‑232, Befund des Auftraggebers auf `/mandantenauswahl`). Mit Suchfeld hält **dessen** `md:ml-auto` Mandant, Sprache und Nutzermenü rechts, und der Produktname ist ab `md` eine Beschriftung (`md:flex-initial`); ohne Navigation gibt es kein Suchfeld und damit kein `ml-auto` — dann muss der Produktname der Füller (`flex-1`) bleiben, sonst rückt alles nach links an ihn heran. Eine Regel, die selbst eine Klasse ist, und eine Aussage über **Abwesenheit** (kein `ml-auto` im Kopf); ob die Klasse an das Vorhandensein des Suchfelds gebunden ist, sagt nur der Baum. Zwei Mutanten (unbedingtes `flex-initial`, `ml-auto` entfernt), beide rot |
 ...
 *
 * > ⚠️ **Fortgeschrieben am 21.09.2026 (`Escape` in der Dateiansicht, `docs/rohdaten-frontend.md`
 * > §11 Punkt 7)**, aus dem Lauf (`vitest run --reporter=json`, Fälle je Datei): **201 in
 * > vierundzwanzig Dateien**, keine neue rendernde Datei — `tests/artefakt-ansicht.test.tsx` von 19
 * > auf 21. Der Lauf über alle 48 Dateien trägt **1.284** Fälle: genau die zwei hier, keine neue
 * > Quelldatei, also nichts aus `farbwerte` und `serverbausteine`. Die 201 und die 1.284 sind
 * > gezählt; der Basisstand ist nicht eigens gefahren — 1.284 − 2 trifft die 1.282 des Kastens
 * > darüber, und der Zweig ändert keine andere Testdatei. **Dabei berichtigt:** Kopfsatz und Schlusssatz nannten noch
 * > einhundertsiebenundneunzig (Stand 18.09.2026), während der Kasten darüber und die Tabellensumme
 * > schon bei 199 standen; die Tabelle je Datei gegen den Lauf verglichen, einzige Abweichung war
 * > die Zeile dieser Datei.
 *
 ...
 *
 * Allen zweihunderteins ist dasselbe gemeinsam: **Es gibt keinen anderen Ort, an dem sie

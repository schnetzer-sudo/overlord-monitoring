...
| **`kopfzeile.test.tsx`** *(22.09.2026)* | **gerenderter Baum**, zwei Fälle: Mit Suchfeld hält dessen `md:ml-auto` den Rest der Kopfzeile rechts und der Produktname ist ab `md` eine Beschriftung; ohne Navigation gibt es kein Suchfeld, **kein `ml-auto` im Kopf** (Abwesenheit) und der Produktname bleibt der Füller — sonst rückt alles nach links ([`bam-suche.md`](bam-suche.md) §28, zweiter Befund) |

...
| **`rollen-auswahl.test.tsx`** *(15.09.2026)* | **gerenderter Baum.** Die Zählung steht in `vitest.config.mts`. Die Rollenauswahl ist kein natives Feld mehr — eine Aussage über **Abwesenheit** (`<select>`) —, eine gesperrte Rolle ist weder per Klick noch per Taste wählbar, derselbe Wert meldet nichts, und `↓` am geschlossenen Feld öffnet nur ([`benutzerverwaltung-frontend.md`](benutzerverwaltung-frontend.md) §18) |
| **`spaltenwahl.test.tsx`** *(15.09.2026)* | **gerenderter Baum.** Die Zählung steht in `vitest.config.mts`. Die Regel **ist** eine Klasse: die Container-Schwellen von Trefferliste, Benutzertabelle und Katalog — die Zahl in `@min-[…]/<name>:table-cell` als Summe der Mindestbreiten, an den gemessenen Containerbreiten und an jeder Kante keine Spalte unter ihrer Mindestbreite, `td` wie `th`, keine Fensterschwelle an einer Zelle ([`spaltenwahl.md`](spaltenwahl.md) §9). **Seit dem 16.09.2026 auch die Nachrichtenliste** (E‑148), und dort zusätzlich die Bauform unter der Grundmenge: Der Ablauf bekommt den Rest, das Projekt kommt nicht |

...
## 9. Nachtrag vom 15.09.2026 — Teil 2 ist gebaut und gegengeprüft

Der Umbau steht in [`spaltenwahl.md`](spaltenwahl.md) (E‑147, M177), die Rollenauswahl in
[`benutzerverwaltung-frontend.md`](benutzerverwaltung-frontend.md) §18 (Punkt 180). **Die Erhebung aus §7
ist nach Teil 2 wiederholt**, mit Bauform B, beiden Mandanten und denselben Prüfdaten; der Vergleich
vorher gegen nachher steht in `spaltenwahl.md` §7 und kommt aus `vergleich-m177.mjs`.

| Punkt | Stand |
|---|---|
| **114** | fortgeschrieben, **offen** — die Nachrichtenliste ist nicht gebaut. ✔ **Geschlossen am 16.09.2026** (E‑148): nach erneuter Meldung des Auftraggebers gebaut, [`spaltenwahl.md`](spaltenwahl.md) §5.4 |
| **173**, **174**, **175** | **geschlossen**, mit der Zahl in §5 |
| **176**, **177** | **geschlossen**, mit der Zahl in §5 |
| **178** | **offen**, mit dem Vermerk, was ihm fehlt |

**Die Wiederholung braucht seitdem zwei Dinge mehr:** `rahmen.js` wartet im verdeckten Tab über
`MessageChannel` (Chrome drosselt dort verkettete `setTimeout` nach wenigen Minuten auf etwa einen Aufruf
je Minute), und die Messskripte kommen vom Empfänger (`GET /skript/<datei>`), statt als Text in das
Werkzeug eingesetzt zu werden.

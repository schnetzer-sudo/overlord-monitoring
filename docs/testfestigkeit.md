...
| **T-10** | **Über die Farbwerte in `app/globals.css` selbst sichert kein Test etwas zu.** Wer `--status-fehler` von Rot auf Grün stellt, bekommt einen grünen Lauf. Das ist die **Abgrenzung** dieses Tests und kein Versehen — er blockiert Farben in Komponenten —, aber es heißt, dass an der einen Stelle, an der die Farben wirklich stehen, nichts hält. **Was fehlte, ist eine Entscheidung darüber, was dort überhaupt zusicherbar wäre**: Ein Sollwert je Token wäre eine zweite Pflegestelle, und eine Kontrastprüfung ist ein anderer Test (`visuelles-konzept.md` §7a rechnet sie von Hand). — ⚠️ **FORTGESCHRIEBEN am 03.09.2026, nicht gestrichen: T‑10 ist zur Hälfte geschlossen.** Die Entscheidung, die fehlte, ist gefallen — **zusicherbar ist der Kontrast, nicht der Farbwert**. `tests/farbkontrast.test.ts` liest `globals.css` mit dem CSS-Leser aus `tests/hilfe/css-leser.ts` und hält für **beide** Blöcke: jeder Vordergrund ≥ 4,5 : 1 auf seiner Fläche, auf `--card` und auf `--background` (ungerundet **und** aus dem Hexwert); dieselbe Schwelle für die drei Akzentstufen, die Schrift sind; Regel Q3 rechnerisch (`--ueberfaellig` und `--status-fehler` auf derselben Helligkeit); die Vollständigkeit aller fünf Rollen und vier Akzentstufen je Block; die sRGB-Zugehörigkeit mit **einer festgenagelten Ausnahme**; und die **vierzehn Gegenproben** aus `visuelles-konzept.md` §3 und §7a, ohne die die Methode und nicht das Ergebnis die offene Frage wäre. Die Konturen werden **berichtet und nicht zugesichert** — ein Test darauf wäre heute rot und drehte E‑u um. **Die offene Hälfte ist genau die, die dieser Eintrag von Anfang an benennt**, und sie ist jetzt **gemessen statt ausgedacht:** Im Dunkelblock `--status-abgeschlossen` von `oklch(0.75 0.13 166)` auf `oklch(0.75 0.13 27)` gedreht — **Grün wird Rot**, gleiche Helligkeit, gleiche Chroma — läuft grün durch; ebenso, im hellen Block, die beiden **neutralen** Rollen auf ein kräftiges Blau bzw. ein Rot. Dafür bräuchte es den Sollwert je Token, und der wäre die zweite Pflegestelle, die dieser Eintrag als Grund nennt. **Sie bleibt offen, und zwar als Entscheidung und nicht als Rest.** ⚠️ **Der Mutant „Statusfarbe in `globals.css` umgedreht" aus §10.3 fällt dagegen — aber nicht überall und nicht aus dem Grund, den man vermutet:** Ein Tonwechsel an `--status-fehler` fällt in beiden Blöcken und an `--ueberfaellig` ebenso, weil die **vierzehn Gegenproben** das helle Rot, das helle Grün und die vier Akzentstufen über ihre OKLab-Abstände festnageln, die sechs Hexwerte aus §7a das Orange in beiden Blöcken, und Regel Q3 die Helligkeit von Rot an die von Orange bindet. **Die Umfärbbarkeit endet dort, wo eine Gegenprobe hinreicht** — nicht dort, wo jemand sie gezogen hätte. Gegengeprüft mit **dreizehn Mutanten, in beide Richtungen geeicht** (`dunkelmodus.md` §7.4): zwei überleben planmäßig (einer folgenlos, einer *ist* diese offene Hälfte), elf tödlich, `globals.css` danach byte-gleich. ⚠️ **Und eine Durchsicht am selben Tag hat die erste Fassung des neuen Tests an einer Stelle zerlegt**, die dieselbe Lehre schon trug: Er las `:root` und `.dark`, aber **nie `@theme inline`** — `--color-status-fehler: var(--status-abgeschlossen)` blieb **812/812 grün**, jedes `text-status-fehler` hätte die grüne Farbe getragen, und keine Zahl des Tests wäre falsch geworden. Es ist derselbe Mutant, der in `dichte.test.ts` „am weitesten trug"; die Lehre lag vor, die Anwendung fehlte. Geschlossen mit neunzehn Verdrahtungszusicherungen (`dunkelmodus.md` §7.5) | §10.3, [`dunkelmodus.md`](dunkelmodus.md) §7 |
...
| **T-13** | **`tests/thema.test.ts` liest den Quelltext, nicht das Erzeugnis — und an einer Stelle ist genau das die Lücke.** Er sichert zu, dass `@custom-variant dark` **genau einmal** dasteht und in der Blockform **beide** Zweige trifft, jeder mit `@slot`. Was er nicht sehen kann, ist, ob **Tailwind diese Form auch übersetzt**: Der Ersatz jedes `@slot` durch die Deklarationen der Klasse ist eine Eigenschaft des Übersetzers (nachgesehen in `tailwindcss/dist/lib.js`, Fassung **4.3.3**), keine der Datei. Fiele sie in einer späteren Fassung weg, bliebe der Test **grün** und alle vierzig `dark:`-Bausteine aus `components/ui` blieben in **einem** der beiden Zweige hell — und zwar unsichtbar, weil die eigenen Tokens in beiden Zweigen richtig stehen. **Gemessen ist es einmal** — im gebauten CSS **15 verschiedene Selektoren je Zweig, Mengen gleich**, dazu an einem echten `Switch` am laufenden System ([`dunkelmodus.md`](dunkelmodus.md) §17.2) —, **zugesichert ist es nicht.** Was fehlte, ist keine weitere Regex, sondern ein Test, der das **Erzeugnis** liest: `next build` fahren und im gebauten CSS beide Zweige zählen. Das ist ein anderer Test mit anderen Kosten — **dieselbe Ursache wie T‑7 und T‑8: auswerten statt lesen** —, und dass er offenbleibt, ist eine Entscheidung und kein Rest | [`dunkelmodus.md`](dunkelmodus.md) §17.2, §18.2 |

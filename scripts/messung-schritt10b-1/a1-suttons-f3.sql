-- Messung A.1 vor Schritt 10b-1 — Sitzung 6: Fassung 3 — IGNORE INDEX (ProejctIDIDX) auf Message · SUTTONS
...
-- ==========================================================================
-- F3-referenz-ohneCursor · SUTTONS
-- ==========================================================================
SELECT '=== F3-referenz-ohneCursor · Plan des NACKTEN Statements (L15) ===' AS marke;
EXPLAIN
  ...
  FROM GlassfishDB.Message m IGNORE INDEX (ProejctIDIDX)
...
SELECT '=== F3-referenz-ohneCursor · Plan der gemessenen Huelle ===' AS marke;
...
SELECT '=== F3-referenz-ohneCursor · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
...
SELECT 'F3-referenz-ohneCursor' AS fall,
...
-- ==========================================================================
-- F3-referenz-mitCursor · SUTTONS
-- ==========================================================================
SELECT '=== F3-referenz-mitCursor · Plan des NACKTEN Statements (L15) ===' AS marke;
EXPLAIN
  SELECT m.MessageID, m.MessageLastUpdate, m.MessageStatus, m.ProcessID,
         p.ProcessName, pr.ProjectName, s.SOSName, sa.SOSActionName
  FROM GlassfishDB.Message m IGNORE INDEX (ProejctIDIDX)
...
SELECT '=== F3-referenz-mitCursor · Plan der gemessenen Huelle ===' AS marke;
EXPLAIN SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
       SUM(t.ProcessName IS NOT NULL)   AS mit_prozessname,
...
SELECT '=== F3-referenz-mitCursor · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
...
SELECT 'F3-referenz-mitCursor' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
...
-- ==========================================================================
-- F3-ueberfaellig-ohneCursor · SUTTONS
-- ==========================================================================
SELECT '=== F3-ueberfaellig-ohneCursor · Plan des NACKTEN Statements (L15) ===' AS marke;
EXPLAIN
  SELECT m.MessageID, m.MessageLastUpdate, m.MessageStatus, m.ProcessID,
         p.ProcessName, pr.ProjectName, s.SOSName, sa.SOSActionName
  FROM GlassfishDB.Message m IGNORE INDEX (ProejctIDIDX)
...
SELECT '=== F3-ueberfaellig-ohneCursor · Plan der gemessenen Huelle ===' AS marke;
EXPLAIN SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
       SUM(t.ProcessName IS NOT NULL)   AS mit_prozessname,
...
SELECT '=== F3-ueberfaellig-ohneCursor · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
...
SELECT 'F3-ueberfaellig-ohneCursor' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
...
-- ==========================================================================
-- F3-ueberfaellig-mitCursor · SUTTONS
-- ==========================================================================
SELECT '=== F3-ueberfaellig-mitCursor · Plan des NACKTEN Statements (L15) ===' AS marke;
EXPLAIN
  SELECT m.MessageID, m.MessageLastUpdate, m.MessageStatus, m.ProcessID,
         p.ProcessName, pr.ProjectName, s.SOSName, sa.SOSActionName
  FROM GlassfishDB.Message m IGNORE INDEX (ProejctIDIDX)
...
SELECT '=== F3-ueberfaellig-mitCursor · Plan der gemessenen Huelle ===' AS marke;
EXPLAIN SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
       SUM(t.ProcessName IS NOT NULL)   AS mit_prozessname,
...
SELECT '=== F3-ueberfaellig-mitCursor · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
...
SELECT 'F3-ueberfaellig-mitCursor' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf

-- Messung A.1 vor Schritt 10b-1 — Sitzung 15: Fassung 6 · NEXANS · schmalere Fenster und die anderen Filter
...
-- ==========================================================================
-- F6-NEXANS-24h-referenz
-- ==========================================================================
SELECT '=== F6-NEXANS-24h-referenz · Plan (L15) ===' AS marke;
EXPLAIN
  ...
  LIMIT 51;
SELECT '=== F6-NEXANS-24h-referenz · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
...
SELECT 'F6-NEXANS-24h-referenz' AS fall,
...
-- ==========================================================================
-- F6-NEXANS-7d-referenz
-- ==========================================================================
SELECT '=== F6-NEXANS-7d-referenz · Plan (L15) ===' AS marke;
EXPLAIN
  SELECT m.MessageID, m.MessageLastUpdate, m.MessageStatus, m.ProcessID,
         p.ProcessName, pr.ProjectName, s.SOSName, sa.SOSActionName
  FROM GlassfishDB.Message m
  ...
  ORDER BY m.MessageLastUpdate DESC, m.MessageID DESC
  LIMIT 51;
SELECT '=== F6-NEXANS-7d-referenz · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
...
SELECT 'F6-NEXANS-7d-referenz' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
...
-- ==========================================================================
-- F6-NEXANS-30d-fehler
-- ==========================================================================
SELECT '=== F6-NEXANS-30d-fehler · Plan (L15) ===' AS marke;
EXPLAIN
  SELECT m.MessageID, m.MessageLastUpdate, m.MessageStatus, m.ProcessID,
         p.ProcessName, pr.ProjectName, s.SOSName, sa.SOSActionName
  FROM GlassfishDB.Message m
  ...
  ORDER BY m.MessageLastUpdate DESC, m.MessageID DESC
  LIMIT 51;
SELECT '=== F6-NEXANS-30d-fehler · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
SELECT COUNT(*)                         AS zeilen,
       SUM(t.MessageID IS NOT NULL)     AS mit_id,
       SUM(t.ProcessID IS NOT NULL)     AS mit_prozess,
...
SELECT 'F6-NEXANS-30d-fehler' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
...
PREPARE p_n_F6_NEXANS_30d_prozess_gross FROM @s_nackt;
PREPARE p_h_F6_NEXANS_30d_prozess_gross FROM @s_huelle;
SELECT '=== F6-NEXANS-30d-prozess-gross · Plan (L15) ===' AS marke;
EXECUTE p_n_F6_NEXANS_30d_prozess_gross;
SELECT '=== F6-NEXANS-30d-prozess-gross · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
EXECUTE p_h_F6_NEXANS_30d_prozess_gross;
EXECUTE p_h_F6_NEXANS_30d_prozess_gross;
EXECUTE p_h_F6_NEXANS_30d_prozess_gross;
EXECUTE p_h_F6_NEXANS_30d_prozess_gross;
EXECUTE p_h_F6_NEXANS_30d_prozess_gross;
EXECUTE p_h_F6_NEXANS_30d_prozess_gross;
SELECT 'F6-NEXANS-30d-prozess-gross' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
...
DEALLOCATE PREPARE p_n_F6_NEXANS_30d_prozess_gross;
DEALLOCATE PREPARE p_h_F6_NEXANS_30d_prozess_gross;

-- ==========================================================================
-- F6-NEXANS-30d-prozess-klein
-- ==========================================================================
SET @proz = @proz_klein;
SET @s_nackt = CONCAT('EXPLAIN
  SELECT m.MessageID, m.MessageLastUpdate, m.MessageStatus, m.ProcessID,
...
PREPARE p_n_F6_NEXANS_30d_prozess_klein FROM @s_nackt;
PREPARE p_h_F6_NEXANS_30d_prozess_klein FROM @s_huelle;
SELECT '=== F6-NEXANS-30d-prozess-klein · Plan (L15) ===' AS marke;
EXECUTE p_n_F6_NEXANS_30d_prozess_klein;
SELECT '=== F6-NEXANS-30d-prozess-klein · Ergebnis + Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
EXECUTE p_h_F6_NEXANS_30d_prozess_klein;
EXECUTE p_h_F6_NEXANS_30d_prozess_klein;
EXECUTE p_h_F6_NEXANS_30d_prozess_klein;
EXECUTE p_h_F6_NEXANS_30d_prozess_klein;
EXECUTE p_h_F6_NEXANS_30d_prozess_klein;
EXECUTE p_h_F6_NEXANS_30d_prozess_klein;
SELECT 'F6-NEXANS-30d-prozess-klein' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
...
DEALLOCATE PREPARE p_n_F6_NEXANS_30d_prozess_klein;
DEALLOCATE PREPARE p_h_F6_NEXANS_30d_prozess_klein;

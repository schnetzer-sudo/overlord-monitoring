-- Messung M106 - Abschlussmessung, der Prozessfilter bei NEXANS
...
--
-- Der teuerste bekannte Fall des Projekts: 7.459,912 ms in §5a, 7.519,513 ms neu gemessen in
-- M100. Dieselben zwei Prozessmengen, wortgleich hergeleitet.
--
...
SET @s = CONCAT('EXPLAIN ', CONCAT('SELECT SUM(kumuliert.anzahl), MAX(CASE WHEN kumuliert.kumuliert >= 51 THEN kumuliert.stunde END)
...
SET @s = CONCAT('SELECT SUM(kumuliert.anzahl), MAX(CASE WHEN kumuliert.kumuliert >= 51 THEN kumuliert.stunde END)
FROM (SELECT je_stunde.stunde, je_stunde.anzahl,
             SUM(CASE WHEN je_stunde.voll THEN je_stunde.anzahl ELSE 0 END)
                 OVER (ORDER BY je_stunde.stunde DESC) AS kumuliert
      FROM (SELECT r.stunde AS stunde, SUM(r.anzahl) AS anzahl,
...
SET @s = CONCAT('SELECT SUM(kumuliert.anzahl), MAX(CASE WHEN kumuliert.kumuliert >= 51 THEN kumuliert.stunde END) INTO @zeilen, @ug
FROM (SELECT je_stunde.stunde, je_stunde.anzahl,
             SUM(CASE WHEN je_stunde.voll THEN je_stunde.anzahl ELSE 0 END)
                 OVER (ORDER BY je_stunde.stunde DESC) AS kumuliert
      FROM (SELECT r.stunde AS stunde, SUM(r.anzahl) AS anzahl,
...
SELECT 'NEXANS-prozess-gross' AS fall, @zeilen AS zeilen_im_fenster, @ug AS untergrenze_roh,
...
DEALLOCATE PREPARE p_rechne;
SET @ug_wirksam = GREATEST('2025-11-30 04:09:47', COALESCE(@ug, '2025-11-30 04:09:47'));
SELECT 'NEXANS-prozess-klein' AS fall, @zeilen AS zeilen_im_fenster, @ug AS untergrenze_roh,
       @ug_wirksam AS untergrenze,
       TIMESTAMPDIFF(HOUR, @ug_wirksam, '2025-12-30 04:09:47') AS fenster_stunden,
       (@zeilen IS NULL OR @zeilen = 0) AS nullfall, (@ug IS NULL) AS keine_verengung;

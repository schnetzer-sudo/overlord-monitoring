...
SELECT '=== BAM+EAV / NEXANS / haeufigster BAM-Wert UND Message.SNDPRN / 30T ===' AS marke;
SET max_statement_time = 60;
SELECT QUOTE(v) INTO @b FROM (
  SELECT b.MessageBAMValue AS v, COUNT(DISTINCT b.MessageID) AS c
    FROM GlassfishDB.Message m
    JOIN GlassfishDB.MessageBAM b ON b.MessageID = m.MessageID
   ...
   GROUP BY b.MessageBAMValue ORDER BY c DESC, v ASC LIMIT 1) x;
SELECT CHAR_LENGTH(@b) - 2 AS laengeDesPruefwerts_b;
...
SET @q = CONCAT(
 ...
 '`GlassfishDB`.`Message`.`Target` from `GlassfishDB`.`MessageBAM` as `b1` join ',
 '`GlassfishDB`.`MessageProperty` as `mp1` on `mp1`.`MessageID` = `b1`.`MessageID` join ',
 '`GlassfishDB`.`Message` on `GlassfishDB`.`Message`.`MessageID` = `b1`.`MessageID` where ',
 '(`b1`.`MessageBAMValue` in (', @b, ') and `mp1`.`MessagePropertyName` = ''Message.SNDPRN'' and ',
 '`mp1`.`MessagePropertyValue` = ', @w, ' and `GlassfishDB`.`Message`.`MessageLastUpdate` >= ''2025-11-30 ',
...
SELECT '=== EAV+EAV / NEXANS / Message.SNDPRN UND Message.VFN / 30T ===' AS marke;
SET max_statement_time = 60;
SELECT QUOTE(v) INTO @w FROM (
  SELECT p.MessagePropertyValue AS v, COUNT(*) AS c
    FROM GlassfishDB.Message m
...
SELECT QUOTE(v) INTO @v FROM (
  SELECT p.MessagePropertyValue AS v, COUNT(*) AS c
   ...
   WHERE m.MessageLastUpdate >= '2025-11-30 00:00:00' AND m.MessageLastUpdate < '2025-12-30 00:00:00'
     AND EXISTS (SELECT 1 FROM GlassfishDB.Process pr JOIN GlassfishDB.ProjectMandant pm ON pm.ProjectID = pr.ProjectID WHERE pr.ProcessID = m.ProcessID AND pm.MandantID = 'NEXANS')
   GROUP BY p.MessagePropertyValue ORDER BY c DESC, v ASC LIMIT 1) x;
SELECT CHAR_LENGTH(@v) - 2 AS laengeDesPruefwerts_v;
SET max_statement_time = 10;
SET @q = CONCAT(
 'SELECT COUNT(*) AS zeilen, MAX(CHAR_LENGTH(ProcessName)) AS maxPnLaenge FROM (select ',
 '`treffer`.`MessageID`, `treffer`.`MessageLastUpdate`, `treffer`.`MessageStatus`, `treffer`.`ProcessID`, ',
 ...
 '`GlassfishDB`.`MessageProperty` as `mp2` on `mp2`.`MessageID` = `mp1`.`MessageID` join ',
 ...
 '`mp2`.`MessagePropertyName` = ''Message.VFN'' and `mp2`.`MessagePropertyValue` = ', @v, ' and ',
...
SELECT '=== EAV+Typ0 / NEXANS / Message.SNDPRN UND Message.Status / 30T ===' AS marke;
SET max_statement_time = 60;
SELECT QUOTE(v) INTO @w FROM (
  SELECT p.MessagePropertyValue AS v, COUNT(*) AS c
    FROM GlassfishDB.Message m
...
SET @q = CONCAT(
 ...
 '`GlassfishDB`.`Message` on `GlassfishDB`.`Message`.`MessageID` = `mp1`.`MessageID` where ',
 '(`mp1`.`MessagePropertyName` = ''Message.SNDPRN'' and `mp1`.`MessagePropertyValue` = ', @w, ' and ',
 '`GlassfishDB`.`Message`.`MessageStatus` = ', @s, ' and `GlassfishDB`.`Message`.`MessageLastUpdate` >= ',
...
SELECT '=== BAM+EAV / NEXANS / haeufigster BAM-Wert UND Message.SNDPRN / 1J ===' AS marke;
SET max_statement_time = 60;
SELECT QUOTE(v) INTO @b FROM (
  SELECT b.MessageBAMValue AS v, COUNT(DISTINCT b.MessageID) AS c
    FROM GlassfishDB.Message m
...
SET @q = CONCAT(
 ...
 '`GlassfishDB`.`Message` on `GlassfishDB`.`Message`.`MessageID` = `b1`.`MessageID` where ',
 '(`b1`.`MessageBAMValue` in (', @b, ') and `mp1`.`MessagePropertyName` = ''Message.SNDPRN'' and ',
 '`mp1`.`MessagePropertyValue` = ', @w, ' and `GlassfishDB`.`Message`.`MessageLastUpdate` >= ''2024-12-30 ',
 '00:00:00'' and `GlassfishDB`.`Message`.`MessageLastUpdate` <= ''2025-12-30 00:00:00'' and exists (select ',
 '1 as `one` from `GlassfishDB`.`Process` as `mandanten_process` join `GlassfishDB`.`ProjectMandant` on ',
 '`GlassfishDB`.`ProjectMandant`.`ProjectID` = `mandanten_process`.`ProjectID` where ',
 '(`mandanten_process`.`ProcessID` = `GlassfishDB`.`Message`.`ProcessID` and ',
...
SELECT '=== EAV+EAV / NEXANS / Message.SNDPRN UND Message.VFN / 1J ===' AS marke;
SET max_statement_time = 60;
SELECT QUOTE(v) INTO @w FROM (
  SELECT p.MessagePropertyValue AS v, COUNT(*) AS c
    FROM GlassfishDB.Message m
...
SELECT '=== EAV+Typ0 / NEXANS / Message.SNDPRN UND Message.Status / 1J ===' AS marke;
SET max_statement_time = 60;
SELECT QUOTE(v) INTO @w FROM (
  SELECT p.MessagePropertyValue AS v, COUNT(*) AS c
    FROM GlassfishDB.Message m

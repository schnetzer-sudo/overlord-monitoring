-- Messung M107: DIE MONATSEBENE UEBER ZWOELF MONATE · NEXANS
...
-- schnell entstehen.
SELECT '=== Gleichheitsprobe NEXANS: Stunde gegen Tag gegen Monat ===' AS marke;
SELECT s.eimer,
...
SELECT '=== Gleichheitsprobe NEXANS: Eimer und Summen je Ebene ===' AS marke;
SELECT 'stunde' AS ebene, COUNT(*) AS eimer, SUM(nachrichten) AS nachrichten FROM (
  ...
  JOIN GlassfishDB.Project pr ON pr.ProjectID = p.ProjectID
  JOIN GlassfishDB.ProjectMandant pm ON pm.ProjectID = pr.ProjectID
  WHERE pm.MandantID = 'NEXANS' AND r.stunde >= '2025-01-01 00:00:00' AND r.stunde < '2026-01-01 00:00:00'
...
SELECT 'tag', COUNT(*), SUM(nachrichten) FROM (
  ...
  JOIN GlassfishDB.Project pr ON pr.ProjectID = p.ProjectID
  JOIN GlassfishDB.ProjectMandant pm ON pm.ProjectID = pr.ProjectID
  WHERE pm.MandantID = 'NEXANS' AND r.tag >= '2025-01-01' AND r.tag < '2026-01-01'
...
SELECT 'monat', COUNT(*), SUM(nachrichten) FROM (
  ...
  JOIN GlassfishDB.Project pr ON pr.ProjectID = p.ProjectID
  JOIN GlassfishDB.ProjectMandant pm ON pm.ProjectID = pr.ProjectID
  WHERE pm.MandantID = 'NEXANS' AND r.monat >= '2025-01-01' AND r.monat < '2026-01-01'
...
-- ==========================================================================
-- verlauf-monat-12monate-NEXANS
-- ==========================================================================
SELECT '=== verlauf-monat-12monate-NEXANS · Plan (L15) ===' AS marke;
...
SELECT '=== verlauf-monat-12monate-NEXANS · Aufwaermlauf + fuenf ===' AS marke;
...
GROUP BY r.monat, r.message_status
ORDER BY r.monat, r.message_status;
SELECT 'verlauf-monat-12monate-NEXANS' AS fall,
...
-- ==========================================================================
-- verlauf-monat-dateformat-12monate-NEXANS
-- ==========================================================================
SELECT '=== verlauf-monat-dateformat-12monate-NEXANS · Plan (L15) ===' AS marke;
...
SELECT '=== verlauf-monat-dateformat-12monate-NEXANS · Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
...
GROUP BY DATE_FORMAT(r.monat, '%Y-%m-01'), r.message_status
ORDER BY DATE_FORMAT(r.monat, '%Y-%m-01'), r.message_status;
SELECT 'verlauf-monat-dateformat-12monate-NEXANS' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
...
-- ==========================================================================
-- verteilung-monat-12monate-NEXANS
-- ==========================================================================
SELECT '=== verteilung-monat-12monate-NEXANS · Plan (L15) ===' AS marke;
...
SELECT '=== verteilung-monat-12monate-NEXANS · Aufwaermlauf + fuenf ===' AS marke;
SELECT MAX(QUERY_ID) INTO @basis FROM information_schema.PROFILING;
...
FROM (
  ...
  GROUP BY CASE WHEN c.process_id IS NULL                THEN NULL
              ...
              WHEN c.partner IS NULL OR c.partner = '' THEN NULL
              ELSE c.partner END) t;
SELECT 'verteilung-monat-12monate-NEXANS' AS fall,
       COUNT(*)                                       AS anzahl_laeufe,
       MAX(CASE WHEN rn = 1 THEN ms END)              AS aufwaermlauf,
       MIN(CASE WHEN rn > 1 THEN ms END)              AS beste_von_fuenf,
       GROUP_CONCAT(CASE WHEN rn > 1 THEN ms END ORDER BY rn) AS alle_fuenf
